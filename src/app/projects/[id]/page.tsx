...
import { ArrowLeftIcon, CalendarIcon, ClockIcon, CurrencyDollarIcon, PlusIcon, PhotoIcon } from '@heroicons/react/24/outline';
import { CheckCircle, PlayCircle, Clock, ExternalLink, Video, Image as ImageIcon, Upload, X } from 'lucide-react';
...
export default function Page({ params }: { params: { id: string } }) {
  ...
  const handleAddPhoto = (updateIndex: number, file: File) => {
    ...
    setProjectData(prev => ({
      ...prev,
      updates: prev.updates.map((update, index) => 
        index === updateIndex 
        ...
        <div className="bg-white rounded-lg shadow-sm p-8 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-amber-50 rounded-lg">
              <p className="text-2xl font-bold text-amber-700 capitalize">{project.status.replace('-', ' ')}</p>
              <p className="text-sm text-gray-600">Status</p>
            </div>
            <div className="text-center p-4 bg-amber-50 rounded-lg">
              <p className="text-2xl font-bold text-orange-700 capitalize">{project.category}</p>
              <p className="text-sm text-gray-600">Category</p>
            </div>
            <div className="text-center p-4 bg-amber-50 rounded-lg">
              <p className="text-2xl font-bold text-amber-600">{project.actualHours || project.estimatedHours}h</p>
              <p className="text-sm text-gray-600">Time Spent</p>
            </div>
            <div className="text-center p-4 bg-amber-50 rounded-lg">
              <p className="text-2xl font-bold text-green-600">${totalMaterialCost.toFixed(2)}</p>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {project.materials.map((material, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 text-gray-900">{material.quantity}</td>
                        <td className="px-6 py-4 text-gray-900">${material.cost.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
